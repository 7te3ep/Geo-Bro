export class Gbro {
   ...
   async init() {
      ...
      const isNotAccessible = ["/entry","/hostLobby","/chooseMap","/speedrun","/classic","/lobby","/championnat"].includes(location.pathname)
      if (isShareLink){
   ...
   async initLinks() {
      document.querySelectorAll("a").forEach((link) => {
         ...
         link.addEventListener("click",async (event)=>{
            ...
            if (path){
               this.loader(true);
               const isGameNav = ["/classic","/speedrun","/hostLobby","/chooseMap","/championnat"].includes(path)
